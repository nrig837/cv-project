...
void getLaplacianPyramid(const Mat_<Vec3f>& image, vector<Mat_<Vec3f> >& laplacianPyramid,
                         int numLevels);
void getGaussianPyramid(const Mat_<float>& mask, vector<Mat_<Vec3f> >& gaussianPyramidMask,
                        int numLevels);
void getBlended(const vector<Mat_<Vec3f> >& targetLapPyr, const vector<Mat_<Vec3f> >& overlayLapPyr,
                const vector<Mat_<Vec3f> >& mask, Mat_<Vec3f>& blendedImage,
                int numLevels);

int main(int argc, char **argv) {
    if (argc != 3) {
        cout << "Incorrect number of arguments ya cunt." << endl;
        return -1;
    }
    
    // Read in images to blend
    Mat overlay = imread(argv[1], CV_LOAD_IMAGE_COLOR);
    Mat target = imread(argv[2], CV_LOAD_IMAGE_COLOR);
    // TODO: check input image dimensions are valid (i.e. target >> overlay)
    // TEMP: initial value range
    double minVal, maxVal;
    minMaxIdx(target, &minVal, &maxVal);
    cout << "Min, max in target initially: " << minVal << ", " << maxVal << endl;
    overlay.convertTo(overlay, CV_32F);
    target.convertTo(target, CV_32F);
    // TEMP: post-conversion value range
    minMaxIdx(target, &minVal, &maxVal);
    cout << "Min, max in target after conversion to float: " << minVal << ", " << maxVal << endl;
    // Programmatically specify mask image
    //Mat mask(target.rows, target.cols, CV_32FC3, Scalar(0.0, 0.0, 0.0));
    Mat_<float> mask(target.rows, target.cols, 1.0);
    // TODO: assert overlay is small enough to center in target
    // TEMP: have a centerline cutoff mask for now
    ...
    // TEMP: output padded overlay and target
    /*namedWindow("Padded Overlay", CV_WINDOW_AUTOSIZE);
    namedWindow("Target", CV_WINDOW_AUTOSIZE);
    normalize(overlay_padded, overlay_padded, 0, 1, NORM_MINMAX, CV_32FC1);
    normalize(target, target, 0, 1, NORM_MINMAX, CV_32FC1);
    imshow("Padded Overlay", overlay_padded);
    imshow("Target", target);
    waitKey(100);
    cout << "overlay_padded rows, cols: " << overlay_padded.rows << ", " << overlay_padded.cols << endl;
    cout << "target rows, cols: " << target.rows << ", " << target.cols << endl;*/
    ...
    Mat_<Vec3f> blendedImage;
    getBlended(targetLapPyr, overlayLapPyr, gaussianPyramidMask,
               blendedImage, numLevels);
    
    
    // Display each level of the mask pyramid
    for (int level = 0; level < numLevels; level++) {
        stringstream levelName;
        levelName << "Level " << level;
        namedWindow(levelName.str(), CV_WINDOW_AUTOSIZE);
        // Scale to original size for comparison
        Mat_<Vec3f> scaled = gaussianPyramidMask[level];
        for (int i = level; i > 0; i--) {
            pyrUp(scaled, scaled, gaussianPyramidMask[i-1].size());
        }
        imshow(levelName.str(), scaled);//gaussianPyramidMask[level]);
    }
    
    // Display each level of the laplacian pyramid
    for (int level = 0; level < numLevels; level++) {
        stringstream levelName;
        levelName << "Laplacian Level " << level;
        namedWindow(levelName.str(), CV_WINDOW_AUTOSIZE);
        // Scale to original size for comparison
        Mat_<Vec3f> scaled = targetLapPyr[level];
        for (int i = level; i > 0; i--) {
            pyrUp(scaled, scaled, targetLapPyr[i-1].size());
        }
        // TEMP: add constant for improved visibility + increased contrast
        double minVal, maxVal;
        minMaxIdx(scaled, &minVal, &maxVal);
        cout << "Min, max at level " << level  << ": " << minVal << ", " << maxVal << endl;
        //targetLapPyr[level].convertTo(targetLapPyr[level], -1, 5, 0);
        normalize(scaled, scaled, 0, 1, NORM_MINMAX, CV_32FC1);
        imshow(levelName.str(), scaled);
    }
    
    // Display blended output
    namedWindow("Blended Image", CV_WINDOW_AUTOSIZE);
